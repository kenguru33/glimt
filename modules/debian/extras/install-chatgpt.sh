...
APP_DIR="$HOME/.local/share/chatgpt-pwa"
...
install_deps(){
  ...
  sudo apt update
  sudo apt install -y curl wget xdg-utils desktop-file-utils librsvg2-bin imagemagick
}
...
download_file(){ # $1=url $2=dest
  local url="$1" dest="$2"
  if have curl; then curl -fsSL -o "$dest" "$url" && return 0; fi
...
ensure_png_from_any(){ # $1=input(any), $2=output_png(256x256)
  ...
  case "$mime" in
    image/png)   if have magick; then magick "$in" -resize 256x256 "$out"; else convert "$in" -resize 256x256 "$out"; fi ;;
    image/svg+xml) rsvg-convert -w 256 -h 256 "$in" -o "$out" ;;
    image/webp|image/x-icon|image/vnd.microsoft.icon|application/octet-stream|"")
      if have magick; then magick "$in" -resize 256x256 "$out"; else convert "$in" -resize 256x256 "$out"; fi ;;
    *)
      if have magick; then magick "$in" -resize 256x256 "$out"; else convert "$in" -resize 256x256 "$out"; fi ;;
  esac
}
...
fetch_and_soften_icon(){
  mkdir -p "$APP_DIR"
  rm -f "$ICON_TMP" "$ICON_RAW" "$ICON_SVG" "$MASK" "$CARD" "$INNER"
  ...
  if [[ ! -s "$ICON_RAW" ]]; then
    ...
    rsvg-convert -w 512 -h 512 "$ICON_SVG" -o "$ICON_RAW" || die "Failed to render SVG to PNG."
  fi
  ...
  if have magick; then
    magick "$ICON_RAW" -resize "${inner}x${inner}" -background none -gravity center -extent "${inner}x${inner}" "$INNER"
    # rounded alpha mask
    magick -size "${CARD_SIZE}x${CARD_SIZE}" xc:none \
    ...
    magick -size "${CARD_SIZE}x${CARD_SIZE}" xc:white "$MASK" -compose CopyOpacity -composite "$CARD"
    # composite inner onto card
    local ts="$(date +%s)"
    ICON_OUT="$APP_DIR/chatgpt-rounded-${CARD_SIZE}-p${CARD_PAD}-r${CARD_RADIUS}-${ts}.png"
    magick "$CARD" "$INNER" -gravity center -compose over -composite "$ICON_OUT"
  else
    convert "$ICON_RAW" -resize "${inner}x${inner}" -background none -gravity center -extent "${inner}x${inner}" "$INNER"
    convert -size "${CARD_SIZE}x${CARD_SIZE}" xc:none \
      -fill white -draw "roundrectangle 0,0 $((CARD_SIZE-1)),$((CARD_SIZE-1)) ${CARD_RADIUS},${CARD_RADIUS}" \
      "$MASK"
    convert -size "${CARD_SIZE}x${CARD_SIZE}" xc:white "$MASK" -compose CopyOpacity -composite "$CARD"
    local ts="$(date +%s)"
    ICON_OUT="$APP_DIR/chatgpt-rounded-${CARD_SIZE}-p${CARD_PAD}-r${CARD_RADIUS}-${ts}.png"
    convert "$CARD" "$INNER" -gravity center -compose over -composite "$ICON_OUT"
  fi

  rm -f "$ICON_TMP" "$MASK" "$CARD" "$INNER"
...
# -------------------------------
configure(){
  if have update-desktop-database; then update-desktop-database "$LAUNCHER_DIR" || true; fi
  case ":$PATH:" in *":$HOME/.local/bin:"*) ;; *)
    log "Note: add '\$HOME/.local/bin' to PATH to use 'chatgpt'/'cgpt' from the terminal." ;;
  esac
  log "Done."
}
