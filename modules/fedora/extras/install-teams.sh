...
# Microsoft Teams Web PWA (Chrome/Chromium/Brave) — Fedora only
...
APP_DIR="$HOME_DIR/.local/share/teams-pwa"
PROFILE_DIR="$HOME_DIR/.local/share/teams-chrome-profile"
...
fetch_official_icon(){
  ...
  if download_file "$OFFICIAL_SVG_MAIN" "$ICON_SVG" && [[ -s "$ICON_SVG" ]]; then
    ...
    if command -v rsvg-convert >/dev/null 2>&1; then
      ...
      die "librsvg2-tools (rsvg-convert) missing — cannot convert Teams SVG to PNG."
    fi
  fi
...
if [[ -z "${BROWSER:-}" ]]; then
  echo "teams: No supported browser (Chrome/Chromium/Brave) found." >&2
  exit 127
fi
