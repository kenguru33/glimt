#!/usr/bin/env zsh
# Delete a saved session from the list at ~/.local/share/kitty/sessions/.
# Bound to ctrl+shift+backspace in kitty.conf via `launch --type=overlay`, so it runs in a
# transient overlay window with a TTY for the interactive fzf picker. fzf is a
# core glimt module (install-fzf.sh), so it is always installed.
set -euo pipefail
...
sel="$(print -l "${files[@]:t}" | fzf --prompt='Delete session > ' --height=100%)" || exit 0
[[ -n "$sel" ]] || exit 0

rm -f "$SESSIONS_DIR/$sel" && notify "$sel" "Session deleted"
