#!/usr/bin/env bash
# Glimt module: kubectx & kubens (Kubernetes context/namespace switchers)
# Actions: all | deps | install | config | clean

set -Eeuo pipefail
trap 'echo "❌ kubectx module failed." >&2' ERR
...
ZSH_FILE="$ZSH_CONFIG_DIR/kubectx.zsh"
...
install() {
  ...
  log "🔌 Installing kubectx and kubens via Homebrew..."

  if brew list kubectx &>/dev/null; then
    log "🔄 kubectx already installed, upgrading..."
    brew upgrade kubectx
  else
    log "⬇️  Installing kubectx..."
    brew install kubectx
  fi

  if ! command -v kubectx &>/dev/null 2>&1; then
    log "❌ kubectx command not found after installation"
    exit 1
  fi

  log "✅ kubectx is ready: $(command -v kubectx)"
  log "✅ kubens is ready: $(command -v kubens)"
}

config() {
  require_user

  log "🔧 Configuring kubectx/kubens (aliases)..."

  if ! command -v kubectx &>/dev/null 2>&1; then
    log "❌ kubectx command not found. Run 'install' first."
    exit 1
  fi

  if ! command -v kubens &>/dev/null 2>&1; then
    log "❌ kubens command not found. Run 'install' first."
    exit 1
  fi
...
EOF
  ...
  log "✅ kubectx configuration complete"
}

clean() {
  require_user

  log "🧹 Removing kubectx/kubens config..."
  ...
  # Uninstall via Homebrew if available
  if check_brew && brew list kubectx &>/dev/null; then
    log "🔄 Uninstalling kubectx via Homebrew..."
    brew uninstall kubectx
    log "✅ kubectx uninstalled"
  else
    log "ℹ️ kubectx not installed via Homebrew (or brew not available)"
  fi
